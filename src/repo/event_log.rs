...
impl EventLogRepository {
    ...
    pub async fn get_events_for_user(
        ...
        let min_event_id = query!(
            "select min(event_id) as 'id: i64' from photos_event_log where user_id = $1 or user_id is null",
            user_id,
        )
        ...
        let event_logs = query_as!(
            EventLog,
            "select event_id, photo_id, data from photos_event_log where event_id > $1 and (user_id = $2 or user_id is null) order by event_id",
            last_event_id,
            user_id,
        )
            .fetch_all(tx.as_mut())
